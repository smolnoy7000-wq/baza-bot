...
- бот НЕ отправляет уведомления в группу;
- "забрать ..." распознаётся по началу сообщения;
- панель без ручного изменения статуса;
- удаление заказа только по паролю 8989;
- новая простая панель из 4 папок.
============================================================
*/
...
function looksLikeOrder(text) {
    ...
    if (
        /2gis\.kz|maps\.google|yandex|geo:/i
            .test(t)
    ) {
    ...
    if (
        /(генератор|гильотин|резка|мойка|компрессор|дрель|перфоратор|свар|лестниц|бош|bosch|бетономеш|инструмент)/i
            .test(t)
    ) {
...
// ВАЖНО:
//
// Функция notifyGroup намеренно НЕ ОТПРАВЛЯЕТ НИЧЕГО.
//
// Раньше бот мог писать:
//
// "Заказ #123 у клиента"
// "Заказ #123 нужно забрать"
// "Заказ #123 забран"
//
// Теперь этого НЕТ.
// ============================================================
...
async function handleIncomingMessage(
    ...
    /*
    Исходящие личные сообщения бота
    не обрабатываем как команды.
    */
